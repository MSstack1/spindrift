...
class HealingPotion:
    ...
    def update(self):
        if self.active == True:
            distance = ((player.pos[0] - self.pos[0])**2 + (player.pos[1] - self.pos[1])**2)**0.5  # Check distance to the player
...
class RangedEnemy:
    ...
    def range_check(self, player):  
        ...
        if (distance < 250 and self.attack_cooldown <= 0): # Check if Player is within 250 pixels, cast fireball
            fireball = bouncingObject((self.pos[0],self.pos[1]), direction, 10, 4)
